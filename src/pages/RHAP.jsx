...
const RHAP = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://localhost:8443/ctx/run/DEATE%20-%20relatorios%20gerenciais/deatesAPI",
      { signal }
    )
      .then(res => res.json())
      .then(resposta => {
  ...
  useEffect(() => {
    const requestList = selectedYears.map(ano => {
      const url =
        "https://localhost:8443/ctx/run/DEATE%20-%20relatorios gerenciais/periodosDisponiveis?" +
        "ano=" +
        ano +
        "&mes=" +
        selectedMonth;
  ...
  const selectDeateHandler = deate => {
    ...
    const url =
      "https://localhost:8443/ctx/run/DEATE%20-%20relatorios gerenciais/equipesAPI?" +
      "unidade=" +
      deate.codigo +
      "&dataInicial=" +
      maiorRhap["dataInicial"] +
      "&dataFinal=" +
      maiorRhap["dataFinal"];
    ...
    fetch(url)
      .then(resp => resp.json())
      .then(equipes => {
        const fixedEquipes = equipes
          .filter(eq => !eq["sigla"].includes("EXCLUÍDA"))
          .map(eq => {
  ...
  const gerarRelatorio = () => {
    ...
    if (
      selectedYears[0] === new Date().getFullYear() &&
      selectedMonth === String(new Date().getMonth() + 1)
    ) {
      showMessage(
        "Não é possível consultar o mês atual, por favor selecione outro!"
      );
      return;
    }
    ...
    selectedEquipes.forEach(nomeEquipe => {
      ...
      rhaps.forEach(rhap => {
        ...
        if (
          moment2Check.isSame(currentDate, "month") &&
          moment2Check.isSame(currentDate, "month")
        ) {
          return;
        }
        ...
        const url =
          "https://localhost:8443/ctx/run/DEATE%20-%20relatorios gerenciais/usuariosAPI?" +
          "unidade=" +
          selectedDeate["codigo"] +
          "&equipe=" +
          equipe["codigo"] +
          "&dataInicial=" +
          rhap["dataInicial"] +
        ...
        fetch(url)
          ...
          .then(usuarios => {
            usuarios.forEach((usuario, idx) => {
              ...
              const url =
                "https://localhost:8443/ctx/run/DEATE%20-%20relatorios gerenciais/rhap?" +
                "unidade=" +
                selectedDeate["codigo"] +
                "&equipe=" +
                equipe["codigo"] +
              ...
              fetch(url)
                ...
                .then(resposta => {
                  if (!resposta["error"]) {
                    ...
                    setListaErro(prevListaErro => [...prevListaErro, ...erro]);
                  }
  ...
  useEffect(() => {
    if (count === totalConsulta) {
      ...
      if (totalConsulta >= 1 && count >= 1) {
        resetState();
        listaResultado.forEach(res => console.log(Object.keys(res).length));
        exportToExcel(listaResultado, listaErro);
      }
    }
    ...
    <>
      {isConsultando && (
        <LoadingTableScreen
          ...
          label={
            <p>
              Gerando um total de <span>{totalConsulta}</span> RHAPs.
            </p>
          }
        />
      )}
